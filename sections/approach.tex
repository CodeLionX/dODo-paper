...
%TODO add references
The \ocddiscover{} algorithm creates the search space for finding \glspl{od} over \glspl{ocd}.
...
\Citeauthor{consonni} build the search tree, by testing each candidate for being order compatible.
...
This happens by adding a column that does not yet appear on either side of the candidate once to its left and once to its right side~\cite{consonni}.

\Citeauthor{consonni} proved in their paper, that they would be able to find all minimal \glspl{od} using this method of generating candidates.
