...
\begin{abstract}
  ...
  All of them are designed to run on a single system and have an at least exponential runtime complexity. 
  In this work we introduce an \gls{od} discovery algorithm, called \dodo{}, which distributes the detection of \glspl{od} across several machines in a cluster, thereby improving both speed and scalability. 
  \dodo{} is build to be fault-tolerant and work with a dynamically changing cluster size.
  We evaluate \dodo{} in respect to its performance, scalability and robstness.
  We demonstrate that the single node version of \dodo{} is about twice as fast as the \ocddiscover{} algorithm by \citeauthor{consonni}, which \dodo{} is based on and a significant speed-up to the single node version can be achieved by distributing the algorithm across multiple nodes.
  \dodo{} shows how distribution can be used to increase perfomance for \gls{od} discovery algorithms, which are limited by the CPUs they are run on.
\end{abstract}
