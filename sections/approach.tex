...
\subsection{The \ocddiscover{} algorithm}\label{sec:ocddiscover}
  ...
  Based on this theorem, they then use a breadth-first search strategy to identify \gls{ocd} relations in the dataset to discovery minimal dependencies before longer ones.
  ...
  If it holds, the candidate is further checked for being order dependent, the results are emitted, and child \gls{ocd} candidates are generated.
...
\subsection{The \dodo{} algorithm}\label{sec:dodo}
  ...
  \begin{itemize}
    \item we brake with the level-wise algorithm
    \item instead: single queue
    \item generation works, because each candidate only depends on its parent~\cite{consonni}
    \item each candidate can be checked independently~\cite{consonni}
    \item allows us to parallelize and distribute the \gls{od} validation
    \item \dots
  \end{itemize}

\subsection{\dodo{} architecture}\label{sec:architecture}
